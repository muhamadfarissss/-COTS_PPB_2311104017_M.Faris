...
class _TaskListPageState extends State<TaskListPage> {
  String _filter = "Semua"; // Pilihan: Semua, Berjalan, Selesai, Terlambat
  String _searchQuery = ""; // Variabel untuk menyimpan text pencarian
      ...
      body: Column(
        children: [
          Container(
            ...
            child: Column(
              children: [
                // Search Bar
                TextField(
                  // TAMBAHAN: Logika update state saat mengetik
                  onChanged: (value) {
                    setState(() {
                      _searchQuery = value;
                    });
                ...
                SingleChildScrollView(
                  scrollDirection: Axis.horizontal,
                  child: Row(
                    children: ["Semua", "Berjalan", "Selesai", "Terlambat"].map(
                      (filter) {
                        bool isActive = _filter == filter;
                        return Padding(
                          padding: const EdgeInsets.only(right: 8),
          ...
          Expanded(
            child: Consumer<TaskController>(
              builder: (context, controller, child) {
                // LOGIKA FILTER DAN SEARCH DIGABUNG
                List<Task> filteredTasks = controller.tasks.where((t) {
                  // 1. Cek Status Filter
                  bool matchesStatus = true;
                  if (_filter == "Berjalan")
                    matchesStatus = t.status == "BERJALAN";
                  else if (_filter == "Selesai")
                    matchesStatus = t.status == "SELESAI";
                  else if (_filter == "Terlambat")
                    matchesStatus = t.status == "TERLAMBAT";

                  // 2. Cek Search Query (Judul atau Mata Kuliah)
  ...
  Widget _buildListItem(BuildContext context, Task task) {
    Color dotColor = task.isDone
        ? Colors.green
        : (task.status == "TERLAMBAT" ? AppColors.danger : AppColors.primary);
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    task.title,
                    style: AppTextStyles.section.copyWith(fontSize: 14),
                  ),
